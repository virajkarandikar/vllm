...
class BlockPool:
    ...
    def free_blocks(self, ordered_blocks: Iterable[KVCacheBlock]) -> None:
        ...
        for block in ordered_blocks:
            block.ref_cnt -= 1
            if block.ref_cnt == 0 and not block.is_null:
                # Track block for zeroing when ref_cnt reaches 0, so the worker
                # can zero its GPU tensors before reuse (prevents KV state
                # contamination; critical for Mamba/FastConformer). Fork addition.

...
async def main():
    with use_tmp_bundle_dir(BUNDLE_DIR, TMP_DIR) as model_dir:
        ...
        # load weights from the HF bundle
        weights_path = os.path.join(model_dir, "model.safetensors")
        sd = load_safetensors(weights_path)

        q_w_key = "encoder.layers.0.self_attn.linear_q.weight"
        q_b_key = "encoder.layers.0.self_attn.linear_q.bias"
        k_w_key = "encoder.layers.0.self_attn.linear_k.weight"
        k_b_key = "encoder.layers.0.self_attn.linear_k.bias"
        v_w_key = "encoder.layers.0.self_attn.linear_v.weight"
        v_b_key = "encoder.layers.0.self_attn.linear_v.bias"
        o_w_key = "encoder.layers.0.self_attn.linear_out.weight"
        o_b_key = "encoder.layers.0.self_attn.linear_out.bias"
        p_w_key = "encoder.layers.0.self_attn.linear_pos.weight"
        pos_u_key = "encoder.layers.0.self_attn.pos_bias_u"
        pos_v_key = "encoder.layers.0.self_attn.pos_bias_v"

        pos_u_param = nn.Parameter(sd[pos_u_key].clone())
        pos_v_param = nn.Parameter(sd[pos_v_key].clone())

        nemo_attn = RelPositionMultiHeadAttention(
            n_head=n_heads,
            n_feat=d_model,
            dropout_rate=dropout_rate,
            pos_bias_u=pos_u_param,
            pos_bias_v=pos_v_param,
            use_bias=True,
        ...
        with torch.no_grad():
            ...
            nemo_attn.linear_k.weight.copy_(sd[k_w_key])
            nemo_attn.linear_v.weight.copy_(sd[v_w_key])
            nemo_attn.linear_pos.weight.copy_(sd[p_w_key])

            nemo_attn.linear_q.bias.copy_(sd[q_b_key])
            nemo_attn.linear_k.bias.copy_(sd[k_b_key])
            nemo_attn.linear_v.bias.copy_(sd[v_b_key])

            nemo_attn.linear_out.weight.copy_(sd[o_w_key])
            nemo_attn.linear_out.bias.copy_(sd[o_b_key])

        pos_enc = RelPositionalEncoding(d_model=d_model, dropout_rate=0.0)
        pos_enc.extend_pe(length=STEPS + 1, device=seq_inputs.device, dtype=seq_inputs.dtype)

        with torch.no_grad():
            mask = torch.triu(torch.ones(STEPS, STEPS, dtype=torch.bool), diagonal=1)
            att_left = int(cfg.get("att_left_ctx", 0))
            att_right = int(cfg.get("att_right_ctx", 0))
            W = att_left + att_right
            assert W == 71, f"W != 71: {W}"
            if W > 0:
                idx = torch.arange(STEPS)
                q_idx = idx.view(1, 1, STEPS, 1)
                k_idx = idx.view(1, 1, 1, STEPS)
                allowed = (k_idx <= q_idx) & ((q_idx - k_idx) <= W)  # [1,1,T,T]
                allowed = allowed.view(1, STEPS, STEPS)[0]
                mask = mask | (~allowed)

...
def benchmark_conformer(
    ...
    for i in range(warmup):
        ...
        window = all_frames[:, i_start : i_end, :]  # [B, T, feats]
    ...
    if mode == "eager":
        ...
        static_inp = torch.empty((B, T, feat_dim), dtype=all_frames.dtype, device=DEVICE)
        static_length = torch.full((B,), T, dtype=torch.long, device=DEVICE)

        first_window = all_frames[:, 0 : context_size + 1, :]
...
async def main():
    ...
    parser.add_argument("--feats", type=int, default=512, help="hidden size")
    ...
    nemo_enc, cfg = build_nemo_conformer(
        d_model=d_model, n_layers=n_layers, n_heads=n_heads, ff_mult=ff_mult,
    ...
    fc_cfg = {
        ...
        "blank_id": 0,
        "vocab_size": 1024,
        "frontend": {"sample_rate": None, "n_fft": None, "n_mels": args.feats},
        "tokenizer": {"type": "sentencepiece", "path": "sentencepiece.bpe.model"},
        "notes": "NeMo ConformerEncoder weights for vLLM benchmark"
    }
